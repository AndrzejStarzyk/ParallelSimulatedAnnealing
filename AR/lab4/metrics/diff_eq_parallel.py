...
def scheduler(n_proc, n, humidity, steps, x):
    ...
    for i in range(2, n_proc):
        par_time[i] = parallel(i)

    for i in range(1, n_proc):
        Comm.send((0, -1, 0, [], [], 0.0), dest=i)

    acc = [par_time[1] / par_time[i] if i != 0 else 0 for i in range(n_proc)]
    eff = [par_time[i]/i if i != 0 else 0 for i in range(n_proc)]
    KF = [(1/par_time[i] - 1/i)/ (1-1/i) if i != 0 else 0 for i in range(n_proc)]
